...
int generate_multivariate_normal(int n_threads, gsl_rng **rng,
    ...
    if (n_threads < 1 || dim < 1 || n < 1 || !rng || !x) {
        err = -1;
        goto cleanup;
    }

    /* Handle identity covariance matrix separately */
    if (covariance) {
...
#pragma omp parallel private(thread_id, rng0) num_threads(n_threads)
        {
            ...
            rng0      = rng[thread_id];
#pragma omp for
            for (ptrdiff_t j = 0; j < n * dim; j++)
                x[j] = gsl_ran_ugaussian(rng0);
        }
    }   
...
#pragma omp for
        for (ptrdiff_t j = 0; j < n; j++)
            x[j] = mean + stddev * gsl_ran_ugaussian(rng0);
    }
